...
@Component({
    moduleId: module.id,
    selector: 'cartable-wr-w4Final',
    templateUrl: './cartable-wr-w4final.template.html',
    styleUrls: ['./cartable-wr-w4final.component.css','../../../assets/css/dashboard.css'],
    providers: [WorkerMgmService, CartableService]

})

export class CartableWorkerWait4FinalComponent {
    ...
    showFinalWorkerCartabl() {
        this.loading = true;
        this._CartableService.getFinalWorkers().subscribe(response => {
            this.workers = <Worker[]>response;
            this.wrCartableLabel = this.shared.fwrCartableLabel;
            this.loading = false;
        }, error => {
            this.loading = false;
    ...
    handleWorkerFinalRegistration(worker: Worker, state: VerifyState) {
        this.confirmationService.confirm({
            message: this.shared.confirmText,
            accept: () => {
                this.loading = true;
                worker.verified = state;
                this._CartableService.verifyWorkerFinal(worker).subscribe(response => {
