...
@Component({
    moduleId: module.id,
    selector: 'cartable-wr-w4Doc',
    templateUrl: './cartable-wr-w4doc.template.html',
    styleUrls: ['./cartable-wr-w4doc.component.css','../../../assets/css/dashboard.css'],
    providers: [WorkerMgmService, CartableService]

})

export class CartableWorkerWait4DocComponent {
    ...
    showWorkeW4DocCartabl() {
        this.loading = true;
        this._CartableService.getWrW4Doc().subscribe(response => {
            this.workers = <Worker[]>response;
            this.wrCartableLabel = this.shared.nwr4DocCartableLabel;
            this.loading = false;
        }, error => {
            this.loading = false;
