...
@Component({
    moduleId: module.id,
    selector: 'cartable-ws-w4Final',
    templateUrl: './cartable-ws-w4final.template.html',
    styleUrls: ['./cartable-ws-w4final.component.css','../../../assets/css/dashboard.css'],
    providers: [WorkerStationMgmService, CartableService]

})

export class CartableWorkStationWait4FinalComponent {
    ...
    ngOnInit() {
        ...
        this.showFinalWorkStationCartabl();
    }
    clearFilter() {
    ...
    onSubmitFilterform() {
        ...
        if (!error) {
            ...
            this._CartableService.search(cartableSearch).subscribe(response => {
                let result = response;
                this.workStations = <WorkStation[]>result;
    ...
    showFinalWorkStationCartabl() {
        this.loading = true;
        this._CartableService.getFinalWorkStations().subscribe(response => {
            this.workStations = <WorkStation[]>response;
            this.wsCartableLabel = this.shared.fwsCartableLabel;
            this.loading = false;
        }, error => {
            this.loading = false;
    ...
    handleWorkstationFinalRegistration(workstation: WorkStation, state: VerifyState) {
        this.confirmationService.confirm({
            message: this.shared.confirmText,
            accept: () => {
                this.loading = true;
                workstation.verified = state;
                this._CartableService.verifyWorkStationFinal(workstation).subscribe(response => {
