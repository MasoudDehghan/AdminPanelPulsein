...
@Component({
    ...
    styleUrls: ['../../../assets/css/dashboard.css','users.css']
})

export class UsersMgmComponent implements OnInit {
    activeLabel: string = this.shared.menuItem3SubItem1Label;
    ...
    init() {
        ...
        this.roleList.push({ label: UserRoleEnum[7], value: 7 });
    ...
    rtvUserList() {
        this.loading = true;
        this._UsersService.getSystemUsersList()
            .subscribe(response => {
    ...
    showDialogToAdd() {
        ...
        this.innerPannelGMessage = [];
        this.errorCntrler.gMessage = [];
        this.dialogHeader = this.shared.addNewUserLabel;
        this.form.clearValidators();
        this.form.reset();
        this.uploadedImageName = "";
        this.rePassword = "";
                ...
                this._UsersService.addUser(this.user)
                    .subscribe(response => {
                        ...
                        if (user.userRole.id == UserRoleEnum.AppUserW && user.workerFlag) {
                            ...
                            user.userRole.id == UserRoleEnum.Accountant) {
            ...
            this._UsersService.updatePass(this.user)
                .subscribe(response => {
                    if (this.user.userRole.id == UserRoleEnum.AppUserW && this.user.workerFlag) {
                        ...
                        this.user.userRole.id == UserRoleEnum.Accountant) {
                        susrs[this.findUserIndexBetweenSystemUsers(this.editedUser)] = this.user;
                        this.systemUsers = susrs;
                    }
